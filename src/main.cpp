...
protected:
    // Embedder-owned action interception: route the AVS drawer's
    // VIS_Prev / VIS_Next to the MilkDrop overlay when one is wired.
    // The generic dispatchAction has no idea what these are; without
    // this hook the prev/next buttons in the AVS drawer fire but
    // nothing happens.
    //
    // Note: there is intentionally no VIS_Random action here — real
    // Winamp's Random control is a `<togglebutton cfgattrib="…;Random">`,
    // not an action button.  Toggle state propagates through
    // CfgAttribStore → MilkdropItem::setShuffle (wired in
    // wireMilkdrop()), exactly mirroring Wasabi's model.
    bool interceptAction(const QString &action, const QString &) override {
#ifdef QTAMP_WITH_MILKDROP
        if (m_milkdropItem) {
            const QString A = action.toUpper();
            if (A == QLatin1String("VIS_PREV")) {
                m_milkdropItem->selectPrev();   return true;
            }
            if (A == QLatin1String("VIS_NEXT")) {
                m_milkdropItem->selectNext();   return true;
            }
        }
#else
        Q_UNUSED(action);
...
static qtWasabi::remote::RemoteTransport *makeRemoteTransport(QString &connectUrl) {
    return qtWasabi::head::makeTransport(connectUrl, configPath());
}
  ...
  if (!modernSkinPath.isEmpty()) {
    ...
    // testing whether Maki dispatch alone produces a sane chrome).
    {
        ...
        if (!::getenv("WASABIQT_NO_RUNTIME")) {
            ...
            // Engine↔head runtime wiring (widget-click delegation,
            // named windows, EQ/slider/volume stores, playback status,
            // scripted resize) — one framework call.
    ...
    if (listActions) {
      ...
      for (const auto &probe : probes) {
        const auto *hit = qtWasabi::Layout::hitTest(
            ...
            qtWasabi::head::imageSizeForHitTest,
            &view->registry());
