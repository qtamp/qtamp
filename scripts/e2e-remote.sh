...
# whose --probe answers THROUGH the pylon's control-channel passthrough.
...
PYLON_PORT="${PYLON_PORT:-18801}"
export QTAMP_MUSIC_ROOT="$(dirname "$FIXTURE")"

[ -x "$QTAMP" ] || { echo "no qtamp binary at $QTAMP (build first)"; exit 1; }
[ -f "$ROOT/pylon/.pylon/index.js" ] || {
    echo "pylon not built (cd pylon && npm run build)"; exit 1; }
...
curl -sN "$PBASE/graphql?query=subscription%7BplayerEvents%7D" \
